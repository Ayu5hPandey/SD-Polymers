  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Image Side - Mobile tweak: reduced base padding from p-8 to p-5 md:p-8 */}
        <div className="relative w-full md:w-1/2 p-5 md:p-8 bg-background-dark flex items-center justify-center">
          <Image 
            src={variant.image} 
            alt={variant.name} 
            width={500}
        ...
        {/* Details Side - Mobile tweak: reduced base padding from p-8 to p-5 md:p-8 */}
        <div className="w-full md:w-1/2 p-5 md:p-8 text-text-light bg-background-card">
          <h2 className="text-xl md:text-2xl font-bold text-text-light mb-2">{variant.name}</h2>
          <p className="text-text-secondary text-sm leading-relaxed mb-6">{variant.description}</p>
          
          <h3 className="text-lg font-semibold text-primary mb-4">Specifications</h3>
          
          {/* Specs Table - THEMED - Mobile tweak: overflow-x-auto to prevent horizontal stretching on small phones */}
          <div className="overflow-x-auto border border-border-color/30 rounded-lg">
            <table className="min-w-full divide-y divide-border-color/30">
              <tbody className="divide-y divide-border-color/30">
                {Object.entries(variant.specs).map(([key, value]) => (
                  <tr key={key} className="group hover:bg-background-dark/30 transition-colors">
                    <td className="px-4 py-3 text-sm font-medium text-text-secondary bg-background-dark/20 w-1/3 whitespace-nowrap">{key}</td>
                    <td className="px-4 py-3 text-sm text-text-light font-mono min-w-[150px]">{value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
